 ...
 */
"use strict";
var FixedSizeLinkedList = require("../dist/index");
...
describe('Fixed Size Linked List', function() {
  ...
  it('Should maintain sum for less number of elements', function(){
    for(let i=1; i<=3; i++) {
      linkedList.push(i)
    }
    assert.equal(linkedList.sum, 6)
  })

  it('Should maintain sum for more number of elements', function(){
    for(let i=1; i<=14; i++) {
      linkedList.push(i)
    }
    assert.equal(linkedList.sum, 95)
  })
});